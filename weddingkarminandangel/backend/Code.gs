...
const RSVP_HEADERS = ['Nombres de los invitados', 'Cupos de la invitación', 'Personas confirmadas', 'Estado', 'Mensaje', 'Confirmación enviada a'];
...
function doGet() {
  return jsonResponse_({ ok: true, service: 'weddingangelandkarmin-rsvp', version: 2 });
}
...
function validateRsvp_(p) {
  ...
  const recipients = { karmin: 'Karmín', angel: 'Angel' };
  if (!recipients[p.recipient]) throw new Error('INVALID');
  return {
    ...
    message: p.message.trim(), recipient: recipients[p.recipient]
  };
}
...
function doPost(e) {
  ...
  try {
    ...
    const rowValues = [textCell_(data.names), data.guests, data.confirmed, data.status, textCell_(data.message), data.recipient];
    ...
    // Request references live in cell notes, keeping exactly the six visible columns.
    ...
    if (existingIndex >= 0) {
      const stored = sheet.getRange(existingIndex + 2, 1, 1, 6).getValues()[0];
      if (stored.every(function (value) { return value === ''; })) {
        sheet.getRange(existingIndex + 2, 1, 1, 6).setValues([rowValues]);
        SpreadsheetApp.flush();
        return jsonResponse_({ ok: true, requestId: data.requestId });
      }
      // getValues returns literal text without its apostrophe escape.
      const expected = [data.names, data.guests, data.confirmed, data.status, data.message, data.recipient];
    ...
    sheet.getRange(row, 1, 1, 6).setValues([rowValues]);
    SpreadsheetApp.flush();
    return jsonResponse_({ ok: true, requestId: data.requestId });
  } catch (error) {
